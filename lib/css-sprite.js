...
module.exports = function (opt) {
  ...
  function createCanvas (layerInfo) {
    // Force even-pixel sprite size for retina
    var width = layerInfo.width % 2 === 0 ? layerInfo.width : layerInfo.width + 1;
    var height = layerInfo.height % 2 === 0 ? layerInfo.height : layerInfo.height + 1;
    ...
    lodash.each(layerInfo.items, function (sprite, index) {
      var x = ( sprite.x + opt.margin ) % 2 === 0 ? sprite.x + opt.margin : sprite.x + opt.margin - 1,
          y = ( sprite.y + opt.margin ) % 2 === 0 ? sprite.y + opt.margin : sprite.y + opt.margin - 1;
      ctx.drawImage(sprite.meta.img, x, y, sprite.width - opt.margin * 2, sprite.height - opt.margin * 2);
    });
    return canvas;
  }
